...
describe("decisions-log module", () => {
  ...
  it("updates state.md timestamp on append", () => {
    ...
    expect(
      new Date(stateAfter.frontmatter.last_updated).getTime(),
  ...
  it("fails loudly when decisions.jsonl has invalid JSON or schema", () => {
    ...
    expect(() => readAllDecisions(tempDir)).toThrow(
      /Failed to parse JSON on line 1/,
    );
  });
  ...
  it("fails loudly when decisions.jsonl has schema violation", () => {
    ...
    fs.appendFileSync(
      filePath,
      JSON.stringify({ id: "bad-id", ts: "2026-01-01", type: "bad" }) + "\n",
      "utf-8",
    );

    expect(() => readAllDecisions(tempDir)).toThrow(
      /Invalid decision schema on line 1/,
    );
  });
  ...
  it("queries decisions by text, type, and ref", () => {
    ...
    const resultsConvention = queryDecisions(tempDir, undefined, {
      type: "convention",
    });
    expect(resultsConvention).toHaveLength(1);
    expect(resultsConvention[0].type).toBe("convention");

    const resultsRef = queryDecisions(tempDir, undefined, {
      ref: "package.json",
    });
