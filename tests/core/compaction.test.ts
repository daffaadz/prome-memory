...
import { appendDecision, readAllDecisions } from '../../src/core/memory/decisions-log.js';
import { readStateFile, incrementSessionCount } from '../../src/core/memory/state-file.js';
import {
  defaultCompactState,
  isCompactionNeeded,
  executeCompaction,
...
describe('Compaction Engine', () => {
  ...
  describe('Pure function defaultCompactState', () => {
    it('integrates decisions into standard markdown sections', () => {
      ...
      expect(compacted).toContain('Modul: vitest.config.ts -> Use Vitest for testing');
      expect(compacted).toContain('## Keputusan aktif');
      expect(compacted).toContain('[d-0001] Use Vitest for testing (Alasan: Fast runner)');
      expect(compacted).toContain('## Konvensi yang sudah disepakati');
      expect(compacted).toContain('[d-0002] Conventional Commits (Alasan: Automated changelogs)');
    });
  });
