...
describe('CLI install, status, and amend commands', () => {
  ...
  describe('runInstall', () => {
    it('installs a skill from a local directory into .prome/skills/', async () => {
      ...
      fs.writeFileSync(path.join(mockSkillSource, 'SKILL.md'), '# Custom Skill Content', 'utf-8');

      const result = await runInstall(mockSkillSource, { cwd: tempDir, json: true });
      expect(result.status).toBe('success');
      expect(result.skillName).toBe('mock-custom-skill');

      const installedSkillPath = path.join(tempDir, '.prome', 'skills', 'mock-custom-skill', 'SKILL.md');
      expect(fs.existsSync(installedSkillPath)).toBe(true);
      expect(fs.readFileSync(installedSkillPath, 'utf-8')).toBe('# Custom Skill Content');
    });

    it('returns error when local source does not exist', async () => {
      const nonExistent = path.join(tempDir, 'non-existent-skill');
      const result = await runInstall(nonExistent, { cwd: tempDir, json: true });
