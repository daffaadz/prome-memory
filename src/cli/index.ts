...
program
  .command("init")
  .description(
    "Initialize Prome memory in the current project (silent and idempotent)",
  )
...
program
  .command("status")
  .description(
    "Show current memory status, token metrics, and compaction triggers",
  )
  .option("--json", "Output result as JSON")
  .action(async (opts) => {
...
program
  ...
  .description(
    "Recall decisions from decisions.jsonl by keyword, type, or file ref",
  )
  .option(
    "-t, --type <type>",
    "Filter by type (architecture | convention | scope)",
  )
...
program
  ...
  .option(
    "-t, --type <type>",
    "Decision type (architecture | convention | scope)",
  )
